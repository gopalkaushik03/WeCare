...
export const api = {
    ...
    // ------------------------------------------------------------------
    analysis: {
        submit: async (mood, notes, cognitiveLoadScore = null) => {
            try {
                const payload = { mood, notes };
                if (cognitiveLoadScore !== null) {
                    payload.cognitive_load_score = cognitiveLoadScore;
                }
                const data = await apiFetch(
                    `/api/v1/analyze?user_id=${DEFAULT_USER}`,
