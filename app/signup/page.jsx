                    ...
                    <form className="space-y-5" onSubmit={async (e) => {
                        ...
                        <div className="space-y-2 group">
                            ...
                            <div className="relative">
                                <User className={`absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 transition-colors ${error && !name ? 'text-red-400' : 'text-muted-foreground group-focus-within:text-primary'}`} />
                                <input
                                    type="text"
                                    value={name}
                                    onChange={(e) => setName(e.target.value)}
                                    placeholder="Joh Doe"
                                    className={`w-full bg-background/50 border rounded-xl py-3 pl-12 pr-4 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all duration-300 shadow-inner ${error && !name ? 'border-red-400/50 bg-red-400/5' : 'border-border'}`}
                                />
                            </div>
                        </div>
                        ...
                        >
                            {isLoading ? "Creating account…" : (<>Start Journey <ArrowRight className="w-4 h-4" /></>)}
                        </motion.button>
